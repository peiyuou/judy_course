...
function ScrollBG() {
    ...
    document.body.onmousemove = function (event) {
        ...
        //左边距和上边距的值可以随意调整
        oLi[0].style.marginLeft = -(x - mx) / 70 * oLi[0].style.zIndex + 'px';
        // oLi[0].style.marginTop = (y - my) / 90 * oLi[0].style.zIndex + 'px';

        oLi[1].style.marginLeft = (x - mx) / 40 * oLi[1].style.zIndex + 'px';
        // oLi[1].style.marginTop = (y - my) / 90 * oLi[1].style.zIndex + 'px';

        oLi[2].style.marginLeft = (x - mx) / 80 * oLi[2].style.zIndex + 'px';
